...
/* Validations */
import { isStringValid, isEmailValid } from '@utils/validations'
...
const GetInTouch: React.FC = () => {
  ...
  return (
    <SC.Section data-section-name="getInTouch">
      <SC.Title data-effect="fade-in">{getInTouch.title}</SC.Title>
      <SC.Content>
          ...
          >
            ...
            <SC.Notice data-effect="fade-in">
              {getInTouch.notice.map((part: any, index: number) =>
                part.to ? (
                  <SC.NoticeLink key={index} to={part.to}>
                    {part.label}
                  </SC.NoticeLink>
                ) : (
                  part.label
            ...
            <SC.Button
              ...
              disabled={submitting}
            >
              {submitting ? <Loader /> : getInTouch.form.buttonText}
            </SC.Button>
          </SC.Form>
        </SC.Left>
        <SC.Right>
          <SC.Information>
            <SC.InformationTitle data-effect="fade-in">
              {getInTouch.information.title}
            </SC.InformationTitle>
            <SC.StepContainer>
              {getInTouch.information.steps.map(
                (step: string, index: number) => (
                  <li key={index} data-effect="fade-in">
                    {step}
                  </li>
                ),
              )}
